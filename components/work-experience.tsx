        ...
        {workExperiences.map((exp) => (
          <div key={exp.id} className="border-l-4 border-primary pl-6">
            <h3 className="text-xl font-semibold flex items-center text-gray-900 dark:text-white">
              ...
              {exp.role} <span className="text-gray-600 dark:text-gray-300 ml-2">| {exp.company}</span>
            </h3>
            <p className="text-base text-gray-700 dark:text-gray-400">{exp.duration}</p>
            <ul className="mt-3 list-disc list-inside space-y-2">
              {exp.responsibilities.map((task, index) => (
                <li key={index} className="text-base text-gray-700 dark:text-gray-300">{task}</li>
              ))}
            </ul>
          </div>
        ))}
